...
pinned sources in `adapters/rabbitmq/specification/sources.lock.json`.
...
| `github.com/faustbrian/go-kafka/adapters/gotelemetry` | adapter responsibility | use `github.com/faustbrian/go-rabbitmq-streams/adapters/otel`; separately review Kafka-only spans, attributes, and propagation semantics |
